...
class RealESRGANer():
    ...
    def __init__(self,
                 ...
                 model_path,
                 dni_weight=None,
                 model: RRDBNet | SRVGGNetCompact | None = None,
                 tile=0,
                 tile_pad=10,
                 pre_pad=10,
                 half=False,
    ...
    def pre_process(self, img):
        ...
        # pre_pad
        if self.pre_pad != 0:
            self.img = F.pad(self.img, (0, self.pre_pad, 0, self.pre_pad),
                             'reflect')
    ...
    def tile_process(self):
        ...
        for y in range(tiles_y):
            for x in range(tiles_x):
                ...
                if output_tile == None:
                    return
                self.output[:, :, output_start_y:output_end_y, output_start_x:
                            output_end_x] = output_tile[:, :,
                                                        output_start_y_tile:
                                                        output_end_y_tile,
                                                        output_start_x_tile:
                                                        output_end_x_tile]
    ...
    def enhance(self, img, outscale=None, alpha_upsampler='realesrgan'):
        ...
        if img_mode == 'RGBA':
            ...
            if alpha_upsampler == 'realesrgan':
                ...
                output_alpha = output_alpha.data.squeeze().float().cpu(
                ).clamp_(0, 1).numpy()
                output_alpha = np.transpose(output_alpha[[2, 1, 0], :, :],
                                            (1, 2, 0))
                output_alpha = cv2.cvtColor(output_alpha, cv2.COLOR_BGR2GRAY)
            elif alpha:  # use the cv2 resize for alpha channel
                h, w = alpha.shape[0:2]
                output_alpha = cv2.resize(alpha,
        ...
        if max_range == 65535:  # 16-bit image
            output = (output_img * 65535.0).round().astype(np.uint16)  # type: ignore
        else:
            output = (output_img * 255.0).round().astype(np.uint8)  # type: ignore
