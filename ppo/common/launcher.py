...
class Launcher:
    ...
    def run(self):
        ...
        logger.info(
            f"Env: {cfg.env_name}, Algorithm: {cfg.algo_name}, Device: {cfg.device}"
        )
        ...
        if cfg.mode.lower() == 'train':
            ...
            for i_ep in range(cfg.train_eps):
                agent, ep_reward, ep_step = self.train_one_episode(
                    env, agent, cfg)
                logger.info(
                ...
                if (i_ep + 1) % cfg.eval_per_episode == 0:
                    ...
                    if mean_eval_reward >= best_ep_reward:  # update best reward
                        ...
                        agent.save_model(
                            self.model_dir)  # save models with best reward
            # env.close()
        elif cfg.mode.lower() == 'test':
            for i_ep in range(cfg.test_eps):
                agent, ep_reward, ep_step = self.test_one_episode(
                    env, agent, cfg)
                logger.info(
